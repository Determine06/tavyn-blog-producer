  ...
  const items = contentRecommendation.content_recommendations.map(
    (contentItem) => {
      ...
      return {
        ...
        recommended_title: requireContentRecommendationField(
          contentItem,
          "recommended_title",
        ),
        recommended_page_type: requireContentRecommendationField(
          contentItem,
...
function mapFinalQueryMetrics(metrics: QueryMetrics) {
  return {
...
function requireContentRecommendationField(
  contentItem: ContentRecommendationItem,
  fieldName: "recommended_title" | "recommended_page_type",
): string {
  const value = contentItem.editorial_recommendation[fieldName];

  if (typeof value !== "string" || value.trim().length === 0) {
    throw new Error(
      `Cannot generate company report because content recommendation ${contentItem.recommendation_id} for query ${contentItem.query_id} is missing editorial_recommendation.${fieldName}.`,
    );
  }
